...
def upload_previews(show: str, kitsu: KitsuAPI, folder_filter: str,
    ...
    print(f"\n[SKYFALL] Kitsu Preview {tag}— {show}\n")
    ...
    for top_dir in top_dirs:
        ...
        for shot_code, (editor_path, version) in sorted(editors.items()):
            ...
            task = kitsu.get_task_for_shot(shot_data["id"], "compositing")
            if not task:
                print(f"     {shot_code}  ⚠  COMPOSITING 태스크 없음")
                continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SKYFALL Kitsu Preview Upload (editor MOV → COMPOSITING preview)",
